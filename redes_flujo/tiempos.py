...
# for n in [20, 40, 60, 80, 100, 150, 200, 250]:
#     distancias = generar_distancias(n, 33)
...
# # Escalado de la curva teórica usando el promedio de las razones en todos los puntos
# t_teorico_puntos = np.power(n_array, 5)
# factor_escala = np.mean(t_array / t_teorico_puntos)

# n_teorico = np.linspace(min(tamanios), max(tamanios), 100)
# t_teorico_normalizado = np.power(n_teorico, 5) * factor_escala

# fig, ax2 = plt.subplots()
# ax2.plot(tamanios, tiempos, "o-", label=f"Curva empírica")
# ax2.plot(n_teorico, t_teorico_normalizado, "--", label="Curva teórica de ajuste")
# ax2.set_xscale("log")
# ax2.set_yscale("log")
# ax2.set_xlabel("Cantidad de antenas (n)")
